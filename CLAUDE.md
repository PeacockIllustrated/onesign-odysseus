...
### Fitting schedule (074–077)
| 077 | retire two PMs | Takes Davey and John off the project-manager list 076 seeded. Two paths, because `fitting_jobs.pm_id` is `ON DELETE SET NULL`: a PM with no jobs is deleted, one with jobs is only deactivated — deleting would strip the colour off work they ran, and colour surviving on finished work is what makes it ownership rather than status. The board keeps resolving every PM for colour and filters only the places that ask you to *choose* one (`activePms`) |
...
- **The extra van is shared state, not a preference.** `vans.is_additional` marks the spare; the office toolbar and the **up button on the TV remote** both flip its `is_active`. That lives in the database rather than localStorage precisely so the TV agrees — a fourth column that only one room could see would be worse than none, which is also why the TV switches the real thing rather than hiding a column locally. Switching it off keeps any jobs on it (they reappear when it comes back) and both boards say how many. `setAdditionalVanActive` is therefore the one write in `lib/schedule/actions.ts` gated on a *session* rather than on super-admin: the wall runs on a floor account, and the press moves no work, deletes nothing, shows up everywhere at once and is reversible from either surface.
...
- **A multi-day job says so by being there.** There is deliberately no "Mon–Wed" tag on the card: the job already renders in every cell it covers, so the calendar fill answers the question the tag was answering, and the tag was spending a line of every card on it. `jobCoversDate` / `jobDates` are what make the span visible; the label helper that fed the tag is gone.
...
View state (`?view=`/`?week=`/`?month=`/`?year=`) is a URL param because the server has to know which window of dates to load — and it makes the TV pointable at an exact view. The office board's "TV view" button opens `/schedule/tv` carrying the week currently on screen; there is deliberately no in-place TV toggle, because hiding the board's own controls leaves the portal chrome behind and that is not what a wall TV wants.

**The board also prints, deliberately bigger than everything else does.** `/admin/schedule/print` (`app/(print)/admin/schedule/print/`) is the week on paper for anyone who would rather read it in their hand than on a wall — the office asked for it larger, so it defaults to **A3 landscape** where every other print view in the project is A4, and a "day per sheet" layout hands a single day the whole page at nearly twice the type again. Paper size is chosen on the page rather than left to the browser's print dialog, because that dialog will happily shrink an A3 layout onto A4 and hand back exactly the small type the sheet exists to avoid. It renders from the same `getScheduleBoard` payload and the same pure helpers as the board and the TV, so the paper cannot disagree with either. Two things differ from the screen on purpose: **the PM is named on every card, not only coloured** — card colour is whose job it is, and colour is the first thing an office mono printer throws away — and a multi-day job **states its span on a day sheet**, where there are no other days on the page to read it from (the week sheet still doesn't, because the job prints in every day it covers).

**The TV has its own shell (`app/schedule/tv/TvBoard.tsx`), but not its own grid.** It renders the *same* `WeekView` / `MonthView` / `YearView` the office board does, so the week on the wall cannot drift from the week on the desk — that is the invariant, and it lives in the views. What the TV does not share is the chrome: `ScheduleBoard`'s toolbar, legend, holding panels, dnd context and modals are absent rather than switched off, because a wall screen has no pointer and nobody is standing at it. (`ScheduleBoard` therefore has no `tv` prop any more; it is the office board only.)

Six things are load-bearing on the TV:

- **The header is one line because it is measured, not because it was drawn that way.** The row is laid out at its natural size and `transform: scale()`d to the width the browser actually reports (`fitChromeScale`), and the header box takes the scaled height so the week gets back what the chrome is not using. The wall is where the assumption broke: a Google TV's CSS viewport is ~960px, not the panel's 1920, so a row that sat comfortably on a desk overflowed there — and its failure mode was not a tidy clip but a collapse, the PM key wrapping to one name per line, its nowrap names spilling over the period beside them, and a third of the panel going to chrome. Nothing in the row wraps or shrinks any more; the row scales. The logo, the remote's key, the PM key and where-you-are share a single ~29px row, because every pixel of chrome is a pixel the week does not get. Card colour is whose job it is, so the PM key has to be there — and sized *above* the board's base font, since the grid is scaled up to fill the panel and the header is not: keyed to the same em it ends up smaller on the wall than the cards it decodes. The holding lists ("to be scheduled" / "to be delivered") run along the bottom in a fixed band — on a wall the width is worth more than the height the office board's side rail would take — and each band is rendered **only when it has something in it**, so on a normal day the week gets that height back.
- **Everything in the header is coloured for the *stage*, not for a panel.** `--fg` / `--fg-muted` are panel ink and stay dark in both themes (see the trap above), so a header label using them is near-invisible on the workshop board's near-black backdrop. The header uses `--bg-fg` / `--bg-fg-muted` / `--accent`; the remote chips are transparent outlines rather than `--card` pills, both for that reason and because a hint should not be the brightest thing on the wall.
- **The remote is the only input, and it is mapped for the thing the wall is used for.** Left/right — and the channel keys, which report as PageUp/PageDown — step the **period**, because "what's on next Tuesday" is the question the board gets asked and it should be one press in the obvious direction. Down swaps week ↔ month (`nextTvView`); up switches the spare van's column. Home/Enter returns to today. There is no day view — a week already shows every day — and year is reachable by URL but not by D-pad, so nothing can strand the wall in a heatmap. `lib/schedule/tv.ts` holds the mapping, DOM-free and Vitest-covered.
- **A TV remote is not necessarily a keyboard, so every action is also a button.** The office runs this on a **Google TV**, and Chrome on Android TV binds the D-pad to a virtual mouse *cursor*: the arrow keys never reach the page, and no web API lets a page opt out. The first build shipped the header as a legend — labels naming what each arrow would do, on the reasoning that a wall screen has no pointer — and on the actual TV nothing happened at all. So the header row is real `<button>`s that each *also* name the key that fires them, and both kinds of remote drive one row: press the arrow, or steer the cursor on and hit OK. Both doors go through a single `press(action)` dispatcher in `TvBoard`, so there is one definition of what "next week" means rather than two that can drift.
  - The buttons are sized as **cursor targets** (~34px at 1080p) with a hover state readable from across a workshop, because a cursor somebody is nudging with a D-pad is the least precise pointer there is, and hover is the only feedback before OK. Trimming that padding to tidy the header is how the board stops being drivable.
  - There is a **"today" button** as well as the Home key: a Google TV remote has no Home key, so without it a cursor-driven board has no way back to this week. It is lit while the board is off this week and disabled once there is nothing to go back to.
  - `Enter` means "today" — **unless a header control holds focus**, where it has to mean that control. A remote that sends keys can Tab onto "next week", and having OK jump to today instead would be indefensible.
- **Week and month always point at the same place.** Stepping a week updates the month it falls in (`monthOfWeek`, by the week's Thursday); stepping a month moves to that month's first week (`weekOfMonthStart`, anchored on the **4th** so the week it returns is one the month actually owns). Without that, down-then-up snaps back to whatever month the board opened on, and anchoring on the 1st bounces a month that opens on a Friday into the previous one — there is a round-trip test for every month of several years.
- **One screen, never scrolled, never clipped.** The grid is measured and `transform: scale()`d to the stage, with the wrapper's width set to `100 / scale`% so the scaled result lands back at the panel's full width — a uniformly scaled board otherwise leaves a third of the TV black. It scales **up** as well as down (to `MAX_FIT_SCALE`), so a quiet week fills the panel instead of floating in a void, and there is no lower clamp: a small board still shows Friday, where a clipped one silently does not. (An earlier version collapsed cards to one line below a floor and rotated a spotlight through them; that hid most of the week most of the time, and is gone.)
- **It comes home on its own.** Now that the remote scans weeks, someone can read four weeks ahead and walk away, leaving the wall authoritative and wrong. After `IDLE_HOME_MS` with no press the board returns to this week — but only if a person actually moved it, so a board the office deliberately pointed at a particular week from the "TV view" button stays where it was put.

Realtime was always wired (both boards subscribe to the six tables via `useRealtimeStatus`), but a wall TV needs more than a socket: it also polls on a slow interval and refreshes on `visibilitychange` / `online`, because a browser that has parked a background tab, or a workshop wifi blip, leaves a board that looks authoritative and is wrong. The fail-loudly offline banner is the one interruption the TV is allowed.
...
Onesign's story animations (`public/motion/bloom-sign-story.html`, `sign-making.html`) are self-contained canvas pages, not app routes: no dependencies, every frame a pure function of `t`, `window.renderFrame` + `window.DURATION` for capture. Onesign staff appear as **the 1nesign crew**: mascots made of the Onesign mark with the 1 lit in each role's colour, specified in `public/motion/onesign-crew.html`. The method is two skills in `.claude/skills/`: **`story-motion`** is the portable engine and workflow (a working template with six switchable styles — paper, flat, chalk, neon, blueprint, cut paper — beat sheets, time warps, the character rig, craft rules, render + encode + delivery tooling), usable for any project; **`onesign-motion`** is the Onesign brand pack on top of it (house palette, the 1nesign crew as a drop-in module, the logo paths, the BLOOM reference film, delivery preferences). The repo copies of the tooling are in `scripts/motion/` (`stills.mjs` contact sheets, `render.mjs` frame-exact MP4 masters, `encode.sh` web/share encodes). Use both skills for any new piece or change to one. Rendered videos are never committed.
