...
Five things are load-bearing on the TV:

- **The key decodes the colours.** Card colour is whose job it is, which is unreadable on a wall without the PM key — it sits top-left, with the view and period pushed right so the two never collide. The holding lists ("to be scheduled" / "to be delivered") run along the bottom in a fixed 20vh band: on a wall the width is worth more than the height the office board's side rail would take, and the grid above is measured against what is left rather than overlapping it.

- **The remote is the only input.** Left/right cycles week → month → year (wrapping, since there is no "jump to year" button); up/down — and the channel keys, which report as PageUp/PageDown — step the period; Home/Enter returns to today. `lib/schedule/tv.ts` holds that mapping and the view cycle, DOM-free and Vitest-covered.
...
- **Condensing is the answer to a packed board, not more shrinking.** Below `MIN_FIT_SCALE` the cards collapse to their title line and a spotlight rotates through them, animating one open at a time to show its detail. The scale itself is *not* clamped at that floor: past it the board keeps shrinking to fit, because a small board still shows Friday and a clipped one silently does not. The condense decision is made from the height of the board with every card **open**, and remembered while condensed — asking the same question of the shorter condensed board answers "no", un-condenses, and flaps.
- **The cards condense via context, not props.** `TvDisplayContext` is read by `JobCard`; threading `condensed`/`spotlightId` down through WeekView → DayRow → cell would push TV-only concerns into the shared grid, which is the drift this section exists to prevent. The office board never provides the context and gets the default.
