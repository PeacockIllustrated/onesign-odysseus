...
const ConfigSchema = z.object({
    config: z.object({
        ...
        lightingMode: z.enum(['front', 'halo', 'edge']),
        standoffMm: z.number().min(0),
        modulePitchMm: z.number().positive(),
        rowPitchMm: z.number().positive(),
        wattsPerModule: z.number().positive(),
        cascadeLimit: z.number().int().positive(),
        maxRunLengthMm: z.number().positive(),
        driverHeadroom: z.number().min(0.1).max(1),
        cableSide: z.enum(['left', 'right', 'top', 'bottom']),
        ipRating: z.string().max(20),
        moduleId: z.string().max(60).nullable(),
        canDepthMm: z.number().positive(),
        faceType: z.enum(['standard', 'dark', 'perforated']),
    }),
