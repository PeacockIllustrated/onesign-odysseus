...
export function ExportBar({
    ...
    // Gather this panel's derived pieces and hand them to the shared works-pack
    // builder (also used by the design-request inbox, re-deriving a customer's
    // submitted design). The in-situ render + client logo are captured by the
    // caller; everything else comes straight off the live derivation. Returns
    // aren't modelled on a visualiser design (they come from the dedicated
    // built-up-returns tool), so no returns section is synthesised.
    const buildProductionPackInput = (
    ...
    const buildAndCreatePack = async (): Promise<string | null> => {
        ...
        const input = buildProductionPackInput(insitu, logo);
