...
export function MaintenanceClient({ initialVisits, orgs, contacts, sites }: Props) {
    ...
    const inputCls = 'w-full text-sm border border-neutral-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#4e7e8c]';
                    ...
                    {STATUS_TABS.map((s) => (
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs font-semibold rounded ${
                                tab === s ? 'bg-[#4e7e8c] text-white' : 'bg-neutral-100 text-neutral-600'
                            }`}
                        >
