...
import { DriverManagerPanel } from './components/DriverManagerPanel';
...
export function PlanningPanel({ deliveries, activeDrivers, allDrivers, monday, includeWeekends }: Props) {
        ...
        <div className="mt-4">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                <div className="flex items-center gap-2">
                    <Link href={`/admin/deliveries?week=${prevMonday.toISOString().slice(0, 10)}${includeWeekends ? '&weekends=1' : ''}`}
                    ...
                    <Link href={`/admin/deliveries?week=${nextMonday.toISOString().slice(0, 10)}${includeWeekends ? '&weekends=1' : ''}`}
                        className="p-1.5 rounded border border-neutral-200 hover:bg-neutral-50">
                        <ChevronRight size={16} />
                    </Link>
                </div>
                <div className="flex items-center gap-2">
                    <Link href={`/admin/deliveries?week=${monday}&weekends=${includeWeekends ? '0' : '1'}`}
